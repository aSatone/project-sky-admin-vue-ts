...
.carousel-container {
  width: 100%;
  height: 66vh; /* 上方幻灯片占据页面的 2/3 */
  cursor: pointer; /* 显示手型鼠标，提示可以点击 */
}
...
.divider {
  width: 100%;
  height: 8px; /* 调整为较大的高度，类似于装饰条 */
  background: linear-gradient(to right, #ff0000, #ff6600); /* 渐变色分割线 */
  box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.2); /* 添加阴影效果，让分割线更突出 */
  position: relative; /* 确保分割线在正确的位置 */
}

.bottom-fixed-image {
  width: 100%;
  height: 34vh; /* 下方图片占据页面的 1/3 */
  display: flex;
...
.bottom-fixed-image img {
  width: 100%; /* 拉伸图片，使其宽度占满容器 */
  height: 100%; /* 拉伸图片，使其高度占满容器 */
  object-fit: fill; /* 图片完全填满，不保持比例 */
}
</style>
